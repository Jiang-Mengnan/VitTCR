...
class TcrPepTransform_single(nn.Module):
    ...
    def forward(self, x):
        ...
        W_attn = [0, 0.019801980198019802, 0, 0.009900990099009901, 0.7623762376237624, 0.019801980198019802, 0, 0.15841584158415842, 0, 0.009900990099009901, 0.019801980198019802, 0,0,0,0]
        W_attn = np.array(W_attn)
        W_attn = torch.unsqueeze(torch.from_numpy(W_attn), 1)
        W_attn = W_attn.float()
        x = torch.mul(x, W_attn)
        x = torch.sum(x, 1)
        x = self.head(x)
        return x